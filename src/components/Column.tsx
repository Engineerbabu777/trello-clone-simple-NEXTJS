...
import NewCardForm from "@/src/components/forms/NewCardForm";
import { default as ColumnCard } from "@/src/components/Card";
...
export default function Column({ id, name }: ColumnProps) {
	...
	const setTasksOrderForColumn = useMutation(
		({ storage }, sortedCards: Card[], newColumnId) => {
			// STORAGE , SORTED CARDS, NEW COLUMN ID!

			const idsOfSortedCards = sortedCards.map((c) => c.id.toString()); // CONTAINS IDS OF CARD!

			const allCards: Card[] = [
				...storage.get("cards").map((c) => c.toObject()),
			]; // AFTER GETTING ALL CARDS TRANSFORM TO OBJECT
